...
<head>
    ...
    <title>Edit Data Siswa - SPK Program Studi</title>
    <style>
        /* Form Styles */
        .form-container {
            max-width: 1200px;
...
<body>
    <div class="form-container">
        <h1 class="form-title">Edit Data Siswa</h1>
        
        <form method="POST" action="<?= url('updateSiswa/'.$siswa->id) ?>">
            <div class="form-card">
                <div class="form-group input-icon">
                    <i class="fas fa-user"></i>
                    <input type="text" class="form-input" placeholder="Nama Lengkap Siswa" 
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" name="jurusan" id="jurusan">
                            <?php $i = 0; foreach ($dtJurusan as $value): ?>
                                <option value="<?= $i ?>" <?= $siswa->jurusan == $i ? 'selected' : '' ?>>
                                    <?= $value ?>
                                </option>
                            <?php $i++; endforeach ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" name="minat" id="minat">
                            <?php $i = 0; foreach ($dtMinat as $value): ?>
                                <option value="<?= $i ?>" <?= $siswa->minat == $i ? 'selected' : '' ?>>
                                    <?= $value ?>
                                </option>
                            <?php $i++; endforeach ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" name="ekonomi" id="ekonomi">
                            <?php $i = 0; foreach ($dtEkonomi as $value): ?>
                                <option value="<?= $i ?>" <?= $siswa->ekonomi == $i ? 'selected' : '' ?>>
                                    <?= $value ?>
                                </option>
                            <?php $i++; endforeach ?>
                        </select>
            ...
            <div class="form-actions">
                ...
                <a href="<?= url('daftarSiswa') ?>" class="cancel-btn">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                if (!confirm('Apakah Anda yakin ingin menyimpan perubahan data siswa?')) {
                    e.preventDefault();
                }
            });
        });
